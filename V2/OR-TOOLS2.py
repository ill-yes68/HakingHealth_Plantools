...
class SimpleScheduler:
    ...
    def add_constraints(self):
        ...
        #         ])
        # contrainte 48h ddee reposs consécutif
        for employee in self.employees:
            for start_day in range(6):  # On regarde tous les intervalles de 2 jours
                self.model.Add(
                    sum(self.shift_assignments[(employee, shift)] for shift in self.shifts[start_day:start_day + 2]) == 0
                )
        # Contrainte : un dimanche de repos toutes les 2 semaines
        for employee in self.employees:
            for week_start in range(0, len(self.shifts), 14):  # Périodes de 2 semaines
                sunday_shifts = [self.shift_assignments[(employee, shift)] for shift in self.shifts if 'dimanche' in shift]
