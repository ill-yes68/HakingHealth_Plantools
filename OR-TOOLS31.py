...
class SimpleScheduler:
    ...
    def add_constraints(self):
        ...
        # # Contrainte 6 : Période de repos de 2 jours consécutifs pour chaque employé
        # days_list = list(self.shifts.keys())  # Liste des jours dans le mois
        # for employee in self.employees:
        #     for i in range(len(days_list) - 1):  # Boucle sur chaque paire de jours consécutifs
        #         day1 = days_list[i]
        #         day2 = days_list[i + 1]

        #         # Variable booléenne : l'employé ne travaille pas un jour donné
        #         no_work_day1 = self.model.NewBoolVar(f'no_work_{employee}_{day1}')
        #         no_work_day2 = self.model.NewBoolVar(f'no_work_{employee}_{day2}')

        #         # Contraintes pour définir si l'employé ne travaille pas le jour 1 ou le jour 2
        #         self.model.Add(no_work_day1 == (sum(self.shift_assignments[(employee, day1, period)] for period in self.shifts[day1]) == 0))
        #         self.model.Add(no_work_day2 == (sum(self.shift_assignments[(employee, day2, period)] for period in self.shifts[day2]) == 0))

        #         # Ajout de la contrainte : au moins un des deux jours doit être un jour de repos
        #         self.model.AddBoolOr([no_work_day1, no_work_day2])
...
employees =  [
    'Alice', 'Bob', 'Charlie', 'Illyes', 'Jimmy', 'Donatien', 'Fanny', 'Claire', 'Louis', 'Sophie',
    'Tom', 'Élodie', 'Maxime', 'Chloé', 'Samuel', 'Sarah', 'Nathan', 'Camille', 'Lucas', 'Marie',
    'Hugo', 'Jade', 'Léo', 'Laura', 'Victor', 'Emma', 'Antoine', 'Nina', 'Alexandre', 'Manon',
    'Yacine', 'Inès', 'Pierre', 'Lucie', 'Bastien', 'Pauline', 'Matthieu', 'Aurore', 'Julien', 'Zoé',
    'Damien', 'Mélanie', 'Quentin', 'Julie', 'Théo', 'Lina', 'Gaël', 'Alicia', 'Étienne', 'Anaïs'
]
